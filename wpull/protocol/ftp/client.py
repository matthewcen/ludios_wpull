...
class Session(BaseSession):
    ...
    async def _init_stream(self):
        ...
        self._control_connection = await self._acquire_request_connection(self._request)
    ...
    async def start(self, request: Request) -> Response:
        ...
        await self._prepare_fetch(request, response)

        response.file_transfer_size = await self._fetch_size(request)

        if request.restart_value:
            try:
                await self._commander.restart(request.restart_value)
                response.restart_value = request.restart_value
            except FTPServerError:
                _logger.debug('Could not restart file.', exc_info=1)

        await self._open_data_stream()

        command = Command('RETR', request.file_path)

        await self._begin_stream(command)

        self._session_state = SessionState.file_request_sent

        return response

    async def start_listing(self, request: Request) -> ListingResponse:
        ...
        try:
            await self._begin_stream(mlsd_command)
        ...
        if not self._listing_type:
            ...
            # exception chaining
            await self._begin_stream(list_command)
    ...
    async def _prepare_fetch(self, request: Request, response: Response):
        '''Prepare for a fetch.
        ...
        if connection_closed:
            ...
            await self._control_stream.reconnect()
        ...
        if connection_closed:
            await self._commander.read_welcome_message()

        await self._log_in()

        self._response.request = request

    async def _begin_stream(self, command: Command):
        '''Start data stream transfer.'''
        begin_reply = await self._commander.begin_stream(command)

        self._response.reply = begin_reply

        self.event_dispatcher.notify(self.Event.begin_transfer, self._response)

    async def download(self, file: Optional[IO]=None, rewind: bool=True,
    ...
    async def download_listing(self, file: Optional[IO],
        ...
        await self.download(file=file, rewind=False,
    ...
    async def _open_data_stream(self):
        '''Open the data stream connection.

        Coroutine.
        '''
        async def connection_factory(address: Tuple[int, int]):
            self._data_connection = await self._acquire_connection(address[0], address[1])
            return self._data_connection

        self._data_stream = await self._commander.setup_data_stream(
            connection_factory
        )
    ...
    async def _fetch_size(self, request: Request) -> int:
        '''Return size of file.

        Coroutine.
        '''
        try:
            size = await self._commander.size(request.file_path)
            return size
        except FTPServerError:
            return
